  node("TestMachine-ut") {
    ...
    stage('Uploading Artifact To Jfrog'){
      withCredentials([usernamePassword(credentialsId: 'JfrogArtifacte', passwordVariable: 'JPASSWORD', usernameVariable: 'JUSER')]) {
          sh "tar -czvf go-test-project_${BUILD_NUMBER}.tar.gz /datavolume1/cd-demo ; curl -u$JUSER:$JPASSWORD -T /datavolume1/go-test-project_${BUILD_NUMBER}.tar.gz 'https://golan.jfrog.io/golan/go/go-test-project_${BUILD_NUMBER}.tar.gz'"
      }
    }
    stage("Build") {

  ...
  node("TestMachine-ut") {
    ...
    stage("Unit Test") {
      sh "docker run --rm -v .:/go/src/cd-demo golang go test cd-demo -v --run Unit"
    }
    stage("Integration Test") { 
      try {
        ...
        sh "docker run --rm -v .:/go/src/cd-demo --link=cd-demo -e SERVER=cd-demo golang go test cd-demo -v --run Integration"
      }
      catch(e) {
